  ...
  if (user) {
    const [{ data: solicitacao }, { data: perfil }] = await Promise.all([
      supabase
        ...
        .select(
          "id, status, category, brand, model, storage_gb, color, imei, offer_type, estimated_value_cents, final_value_cents, proposal_expires_at, contract_accepted_name, contract_accepted_at, documento_selfie_uploaded_at, payment_method, payment_pix_key, payment_bank_details"
        )
        .eq("user_id", user.id)
