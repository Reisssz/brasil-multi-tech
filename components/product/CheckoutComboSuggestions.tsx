...
export function CheckoutComboSuggestions() {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-border bg-surface p-5">
      <h2 className="text-sm font-semibold text-foreground mb-1">Aproveite e complete seu pedido</h2>
      <p className="text-xs text-muted mb-4">Adicione e leve junto:</p>
      <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
        {sugestoes.map((s) => (
          <div key={s.id} className="w-32 shrink-0">
            <ComboSuggestionCard sugestao={s} />
          </div>
        ))}
      </div>
    </div>
