...
import { calculateInstallment, formatBRL, PRICING_RULES } from "@/lib/pricing";
...
export function InstallmentSimulator({ products }: { products: Product[] }) {
  ...
  return (
    <section id="simulador" className="mx-auto max-w-7xl px-4 sm:px-6 py-14 scroll-mt-20">
      <Reveal as="div" className="rounded-3xl bg-ink-2 text-ink-foreground overflow-hidden relative">
        <div
          aria-hidden
        ...
        <div className="grid lg:grid-cols-2 gap-10 p-6 sm:p-10">
          <div className="flex flex-col gap-4 min-w-0">
            ...
            <label className="flex flex-col gap-1.5 mt-2 min-w-0">
              <span className="text-xs font-medium text-ink-muted">Produto</span>
              <select
                value={productId}
            ...
            <label className="flex flex-col gap-2 mt-1">
              <span className="text-xs font-medium text-ink-muted">
                Número de parcelas: <span className="text-ink-foreground font-semibold">{installments}x</span>
              </span>
              <input
                type="range"
                min={1}
              ...
              />
              <div className="flex justify-between text-[11px] text-ink-muted/70">
                <span>1x</span>
          ...
          <div className="flex flex-col gap-4 min-w-0">
            <div className="rounded-2xl bg-white text-foreground p-6 flex flex-col gap-4 shadow-[0_12px_32px_rgba(0,0,0,0.35)] transition-transform duration-300">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted">Preço à vista</span>
              </div>
              <span key={priceCents} className="font-display animate-fade-in-fast text-3xl font-bold tabular-nums">
              ...
              <div className="flex items-baseline gap-2">
                <span
                  ...
                  className="font-display animate-fade-in-fast text-3xl font-bold tabular-nums"
                >
