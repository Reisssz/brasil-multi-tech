      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {banners.map((b, i) => (
          <Reveal key={b.title} delay={i * 70}>
            <Link
              href={b.href}
              className="group relative flex aspect-[4/5] sm:aspect-square rounded-2xl overflow-hidden bg-[#1c1408] active:scale-[0.98] transition-transform"
            >
              {/* eslint-disable-next-line @next/next/no-img-element -- decorative background photo, not a content image */}
              <img
                src={b.photo}
                alt=""
                className="absolute inset-0 w-full h-full object-contain p-6 opacity-90 transition-transform duration-500 ease-out group-hover:scale-110"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent" />
              <div className="absolute inset-0 ring-1 ring-inset ring-white/0 group-hover:ring-brand/60 rounded-2xl transition-all" />
              <div className="absolute inset-x-0 bottom-0 p-4 flex flex-col">
                <span className="font-display text-lg sm:text-xl font-bold text-white leading-tight">{b.title}</span>
